...
const indexOutput =
  '<html lang="en-GB"><head><title>My app</title><style>my-app{display:block}</style></head><body><h1><span>Hello world!</span></h1><my-app></my-app><script src="app.1eb5bc77828f11aa0917.js"></script><script src="second-entry-point.3b91ab8c1856d28b4fae.js"></script></body></html>';
...
describe('multi-entry', function multiEntry() {
  ...
  it('works for a multi entry application', done => {
    webpack(config, (err, stats) => {
      ...
      expect(
        assetKeys.filter(key => key.startsWith('second-entry-point') && key.endsWith('.js')).length,
      ).to.equal(1);
      expect(assetKeys.filter(key => key.startsWith('2') && key.endsWith('.js')).length).to.equal(
        1,
      );
